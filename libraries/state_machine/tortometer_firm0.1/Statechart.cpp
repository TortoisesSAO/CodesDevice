...
sc_boolean Statechart::isStateActive(StatechartStates state) const
{
	switch (state)
	{
		...
		case SM_SD_CHECKING :
		{
			return (sc_boolean) (stateConfVector[SCVI_SM_SD_CHECKING] == SM_SD_CHECKING);
			break;
		}
		...
		case SM_SD_NONC0NNECTED :
		{
			return (sc_boolean) (stateConfVector[SCVI_SM_SD_NONC0NNECTED] == SM_SD_NONC0NNECTED);
			break;
		}
		case SM_BAT_LOW :
		{
...
/* Functions for event sd_check_NONok in interface  */
void Statechart::raiseSd_check_NONok()
{
	inEventQueue.push_back(new SctEvent__sd_check_NONok(sd_check_NONok));
        runCycle();
}
void Statechart::internal_raiseSd_check_NONok()
{
	sd_check_NONok_raised = true;
}
...
void Statechart::enact_SM_SD_NONC0NNECTED()
{
	completed = true;
}

void Statechart::enact_SM_BAT_LOW()
{
	completed = true;
}

void Statechart::enact_SM_SHUTDOWN()
{
	completed = true;
}

void Statechart::enact_SM_GPS_NONFIX()
{
	completed = true;
}
...
/* 'default' enter sequence for state SD_CHECKING */
void Statechart::enseq_SM_SD_CHECKING_default()
{
	/* 'default' enter sequence for state SD_CHECKING */
	stateConfVector[0] = SM_SD_CHECKING;
}
...
/* 'default' enter sequence for state SD_NONC0NNECTED */
void Statechart::enseq_SM_SD_NONC0NNECTED_default()
{
	/* 'default' enter sequence for state SD_NONC0NNECTED */
	enact_SM_SD_NONC0NNECTED();
	stateConfVector[0] = SM_SD_NONC0NNECTED;
}
...
/* Default exit sequence for state SD_CHECKING */
void Statechart::exseq_SM_SD_CHECKING()
{
	/* Default exit sequence for state SD_CHECKING */
	stateConfVector[0] = Statechart_last_state;
}
...
/* Default exit sequence for state SD_NONC0NNECTED */
void Statechart::exseq_SM_SD_NONC0NNECTED()
{
	/* Default exit sequence for state SD_NONC0NNECTED */
	stateConfVector[0] = Statechart_last_state;
}
...
sc_integer Statechart::SM_SD_CHECKING_react(const sc_integer transitioned_before) {
	/* The reactions of state SD_CHECKING. */
	sc_integer transitioned_after = transitioned_before;
	if (!doCompletion)
	{ 
		if ((transitioned_after) < (0))
		{ 
			...
			{
				if (sd_check_NONok_raised)
				{ 
					exseq_SM_SD_CHECKING();
					enseq_SM_SD_NONC0NNECTED_default();
					react(0);
					transitioned_after = 0;
				} 
			}
...
sc_integer Statechart::SM_SD_NONC0NNECTED_react(const sc_integer transitioned_before) {
	/* The reactions of state SD_NONC0NNECTED. */
	sc_integer transitioned_after = transitioned_before;
	if (doCompletion)
	{ 
		/* Default exit sequence for state SD_NONC0NNECTED */
...
void Statechart::runCycle() {
	...
	{ 
		...
		clearInEvents();
		dispatch_event(getNextEvent());
	} while ((((((((((battery_charge_completed_raised) || (USB5V_connected_raised)) || (USB5V_NONconnected_raised)) || (battery_level_ok_raised)) || (battery_level_low_raised)) || (sd_check_ok_raised)) || (sd_check_NONok_raised)) || (gps_fixed_ok_raised)) || (gps_fixed_timeout_raised)) || (adq_check_battery_raised));
	isExecuting = false;
}
