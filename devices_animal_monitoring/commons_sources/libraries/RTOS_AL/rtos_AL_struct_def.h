...
typedef struct{
	...
	TaskStruct_t object;      //task strtuct for holding. Not used by all rtos
	...
	float t_s;        		  //Period to execute task (seconds)
	float d_s;        		  //Deadline to execute task (seconds)
	TickType_t c;     	      //Time (Worst-case execution time)/* Minimum inter-arrival for aperiodic */
	TickType_t t;     		  //Period
	TickType_t d;     		  //Deadline
    //***********************************************
	//----------CUSTOM SCHEDULLER PARAMETERS---------------
	TickType_t r;     //Worst-case Response Time, must be calculated by user API
    ...
    uint8_t    s_p;   //Slack Stealling  (SS) priority
}TaskData;
