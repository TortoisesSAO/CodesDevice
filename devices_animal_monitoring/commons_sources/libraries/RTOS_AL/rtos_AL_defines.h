...
//functions definitions
  ...
  #define TaskStruct_t uint8_t  //non used
#endif
#if (!defined(RTOS_POSIX_AND_OTHER))|defined(RTOS_TI)
#include <ti/sysbios/knl/Task.h>
#define TaskHandle_t Task_Handle//TaskHandle_t is the handle of the thread
#define TaskStruct_t Task_Object  //non used
#endif
...
// UInt         key;
  ...
  #define taskENABLE_INTERRUPTS()    UInt key_local=HwiP_disable()      //enable  all hardware & software interrupts
  #define taskDISABLE_INTERRUPTS()   HwiP_enable(key_local)       //disable all hardware & software interrupts
  #define vTaskStart_Scheduler()     BIOS_start()        //starts scheduller
  #define vTaskEnd_Scheduler()       BIOS_exit(0)         //disable all hardware & software interrupts
 #endif
...
#define TASK_SLEEP_FOREVER       4294967294// (~(0 TickType_t))
#if defined(RTOS_TIRTOS)|defined(RTOS_FREERTOS)
//units are expressed in microseconds
/* Convert seconds to tick periods*/
#define SEC_TO(s)              ((uint32_t)((s )* (      config_tick_rate_HZ  )))
#define MILSEC_TO(ms)          ((uint32_t)((ms )*( 1000*config_tick_rate_HZ ) ))
//as ticks are the normal number, do not perform any convertion
#define TICK_TO(tick)                tick

#define TO_SEC(tick)          ((((float)tick) /config_tick_rate_HZ ))
#define TO_TICK(s)            ((uint32_t)((s )* (      config_tick_rate_HZ  )))
/*
