#ifndef MEMORY_STRUCT_RF_LIZZARD_H
#define MEMORY_STRUCT_RF_LIZZARD_H
...
}data_rfdata_lizard;//6 bytes struct

typedef struct{
    data_id_tx_rf               id;//       Sum of all bytes:---------->  21 bytes
	data_rfdata_lizard      	d; //data   300*12=3600 bytes
    uint16_t                    crc;                      //firmware checksum
}mem_rf_tx_data_lizard;// Has to be LEsser than SD_BLOCK_UNIT_SIZE
typedef union {
    mem_rf_tx_data_lizard        m;
    unsigned char                b[sizeof(mem_rf_tx_data_lizard)];//array of x size: SHould test
}mem_rf_tx_data_lizard_union;//union of all structures

typedef struct{
    mem_data_header             head;
    data_id_rx_rf               id_rx;
    data_id_tx_rf               id_tx;  // id from the transmiter
    data_rfdata_lizard          d;      //data from the transmiter
}mem_rf_rx_data_lizard;// Has to be LEsser than SD_BLOCK_UNIT_SIZE

//create an union of all to facilitate manipulation
typedef union {
    mem_rf_rx_data_lizard        m;
    unsigned char                b[RFDATA_BLOCK_UNIT_SIZE];//array of x size: SHould test
}mem_rf_rx_data_lizard_union;//union of all structures
