...
#ifndef LSM9DS1_TYPES_H_
#define LSM9DS1_TYPES_H_

#include "LSM9DS1_Registers.h"
#include <stdint.h>
//#include <ti/drivers/I2C.h>
// The LSM9DS1 functions over both I2C or SPI. This library supports both.
// But Only I2C is coded for the CC1312R microcontroller, so if you want to
// use SPI you will have to add the add the proper functions YOURSELF.
// The interface mode used must be sent to the LSM9DS1 constructor. Use
// one of these two as the first parameter of the constructor.
enum interface_mode
{
    IMU_MODE_DISABLED,//imu communication_driver_hasbeen disabled
    IMU_MODE_SPI,
	IMU_MODE_I2C,
};
...
enum fifomode{
	FIFO_ACCELEROMETER,//ONLY ACCELEROMETER IS ENABLE
	FIFO_GYROSCOPE_ACCELEROMETER,    //BOTH GYROSCOPE AND ACCELEREMOTER IS ENABLE
};
struct gyroSettings
{
	// Gyroscope settings:
...
struct deviceSettings
{
	...
	//uint8_t* i2c;    // pointer to an instance of I2C interface
};
...
struct fifoSettings
{
	bool SLEEP_G;
	bool FIFO_EN;
	uint8_t fiforegister_content;//CTRL_REG9 register content
	uint8_t fifomode;
	uint8_t min_samples;
};
struct IMUSettings
{
	...
	struct	fifoSettings fifo;
};

//
#define FIFO_MAX_BYTES 384 //MAX AMOUNTS OF BYTES OF FIFO BUFFER
struct IMU_3xasis
{int16_t x; //x axis of sensor
 int16_t y;  //y axis of sensor
 int16_t z;  //z axis of sensor
};
struct IMU_mag{struct IMU_3xasis m; // x, y, and z axis readings of the magnetometer
};
struct IMU_gyro_accel
{int16_t gx; //x axis of sensor
 int16_t gy;  //y axis of sensor
 int16_t gz;  //z axis of sensor
 int16_t ax; //x axis of sensor
 int16_t ay;  //y axis of sensor
 int16_t az;  //z axis of sensor
};
struct IMU_accel
{
 int16_t ax; //x axis of sensor
...
//We define a buffer to
union FIFO_buffer_struct{
	struct IMU_gyro_accel gyro_accel[32];
	struct IMU_accel accel[32];
  uint8_t data[FIFO_MAX_BYTES];// total bytes=386: 2 (temp) + 2*3*(32) (accel)+2*3*(32) (gyro)
};
struct read_count
{
  uint8_t read,read_max;
  uint8_t reada,reada_max;
  uint8_t readg,readg_max;
};

enum IMU_SAMPLING_MODE
{
	/////// /////////////////             ACCEL  GYRO  MAG  TEMP
	ACCEL_10HZ_CONTINOUS,     //           10 Hz  n.a.  n.a.  1Hz
	ACCEL_GYRO_15HZ_CONTINOUS,//         14.9 Hz  14.9  n.a.  1Hz
	ACCEL_GYRO_15HZ_CONTINOUS_MAG1HZ,//  14.9 Hz  14.9  1.25  1Hz
	ACCEL_GYRO_15HZ_CONTINOUS_MAG5HZ,//  14.9 Hz  14.9  05    1Hz
	ACCEL_GYRO_15HZ_CONTINOUS_MAG10HZ,// 14.9 Hz  14.9  10    1Hz
	LOWPOWER_ACCEL_SINGLE_CONVERSION,//    1 Hz  n.a.  n.a.  n.a.
	LOWPOWER_MAG_SINGLE_CONVERSION,//      1 Hz  n.a.  n.a.  n.a.
	LOWPOWER_ACCEL_MAG_SINGLE_CONVERSION,//1 Hz  n.a.  n.a.  n.a.
    TURN_DOWN,                           //PWOER OFF THE DEVICE
};
