...
static uint8_t data_write, data_read; //dummy read
#ifdef PRINT_UART
...
void LSM9DS1::init()
{
	...
	settings.gyro.flipX = true;
	settings.gyro.flipY = true;
	settings.gyro.flipZ = true;
	settings.gyro.orientation = 0;//non used
...
int LSM9DS1::begin(uint8_t agAddress, uint8_t mAddress, uint8_t imu_mode )
{
	...
	/* Call I2C driver init functions */
	   ...
	   if (i2c_init(&i2c0_imu) == 0) {return(1);}//did not open the the I2c DRIVER?->RETURN ERROR
	//Set scales values
	 constrainScales();
	...
	if (whoAmIgotten == (whoAmIexpected)){return 0;} //return 0 because is all ok
	else{
	    if ((mTest!=WHO_AM_I_M_RSP)and(xgTest!=WHO_AM_I_AG_RSP)){return 2;} //could not communicate with either sensor
	    if (mTest!=WHO_AM_I_M_RSP){return 3;} //could not communicate with IMU-magnetometer
	    if (xgTest!=WHO_AM_I_AG_RSP){return 4;} //could not communicate with IMU-accel and gyro
	   }
	}
   return 10;//already started by other thread
}
...
void LSM9DS1::initGyro()
{
    ...
    i2c_start(&i2c0_imu);
    I2Csendreceive(0x08,&data_write,1,&data_read,0);//dummy read to clean buffer
...
void LSM9DS1::initAccel()
{
	...
	//uint8_t d=xgReadByte(CTRL_REG6_XL);//for debugging
...
//Disables FIFO
void LSM9DS1::turn_onGyro(){

    settings.gyro.enabled=true;
    initGyro();
}
void LSM9DS1::turn_offGyro(){
    uint8_t sample_rate=settings.gyro.sampleRate;
    if (settings.gyro.enabled==true){
		settings.gyro.enabled=false;
		settings.gyro.sampleRate=0;
		initGyro();
		settings.gyro.sampleRate=sample_rate;
	}

}
void LSM9DS1::turn_onAccel(){

    settings.accel.enabled=true;
    initAccel();
}
void LSM9DS1::turn_offAccel()
{	uint8_t sample_rate=settings.accel.sampleRate;
    if (settings.accel.enabled==true){
		settings.accel.enabled=false;
		settings.accel.sampleRate=0;
		initAccel();
		settings.accel.sampleRate=sample_rate;
	}
}
void LSM9DS1::turn_onMag(){
    settings.mag.enabled=true;
    settings.mag.operatingMode = 0;// magOperatingMode can be 0=continous conversion
    initMag();
}
void LSM9DS1::turn_offMag(){
    uint8_t sample_rate=settings.mag.sampleRate;
	if (settings.mag.enabled==true){
		settings.mag.enabled=false;
		settings.mag.sampleRate=0;
		settings.mag.operatingMode = 2;// magOperatingMode can be 0-2// 2 = power down
		initMag();
		settings.mag.sampleRate=sample_rate;
	   }
}
void LSM9DS1::turn_off()
{
	if (settings.device.commInterface!=IMU_MODE_DISABLED)
	{
	turn_offAccel();
	disableFIFO();
	turn_offGyro();
	turn_offMag();
	settings.device.commInterface=IMU_MODE_DISABLED;//to indicate interface is closed.
	}
}
...
uint16_t LSM9DS1::readFIFO_continous()
{
	...
	//n     read (12 bytes):x accel(2)- y accel(2)- z accel(2)
       ...
       if (settings.gyro.enabled){
	    ...
	    xgReadBytes(OUT_X_L_G, buffer_ag.data, FIFO_read_bytes);//OUT_X_L_G to read gyro & accel
	    }
       }
       ...
       if ((!(settings.gyro.enabled))&(settings.accel.enabled)){
	    ...
	    xgReadBytes(OUT_X_L_XL, buffer_ag.data, FIFO_read_bytes);//OUT_X_L_XL to read accel
	    }
       }
...
void LSM9DS1::readAccGyro() //perform reads
{
    i2c_start(&i2c0_imu);
    I2Csendreceive(0x08,&data_write,1,&data_read,0);//dummy read to clean buffer
