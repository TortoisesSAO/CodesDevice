...
public:
	...
	//To use the FIFO with 1 read mode, we create the following structure to save the data
	FIFO_buffer_struct buffer_ag;
	int8_t  FIFO_samples_available;//amounts of samples aviable in the last check
	int16_t FIFO_read_bytes;//amounts of bytes read in the last attempt
	...
	int begin(uint8_t agAddress = 0, uint8_t mAddress =0, uint8_t fifo_mode=1); //By default use the default I2C addres, and use Wire port
	...
	void readTemp();
	// calcTemp() -- Convert from RAW signed 16-bit value to degrees per second
	float calcTemp(int16_t temp);
	...
	/*turn off gyro and accelerometer, puts them to sleep*/
	void turn_onAccel(void);
	void turn_offAccel(void);
	void turn_onGyro(void);
	void turn_offGyro(void);
	/*turn off magnetometer sensor, puts them to sleep*/
	void turn_offMag(void);
	/*turn on magnetometer sensor with the current setup*/
    void turn_onMag(void);
	/*turn off all the device*/
	void turn_off(void);
	...
	void setFIFO(fifoMode_type fifo_Mode, uint8_t fifoThs);

	// getFIFOSamples() - Get number of FIFO samples
	uint8_t getFIFOSamples();

	//ADDITIONAL FUNCTIONS ADDED
	void enableFIFO_continous();

	void disableFIFO();
    //read FIFO
	uint16_t readFIFO_continous();//read the FIFO in the current state
	void readAccGyro();//read all the data in the registers
	void write_register(uint8_t device, uint8_t register_address, uint8_t register_content_to_w);//allow the user to write a register
protected:
	...
	void init();
	//this functions get and set the i2C addresses of the magnetometer and Accel/Gyro
	void get_m_xg_addresses();
	...
	int8_t mReadByte(uint8_t subAddress);
	...
	int8_t mReadBytes(uint8_t subAddress, uint8_t * dest, uint16_t count);
	...
	int8_t xgReadByte(uint8_t subAddress);
	...
	// Output: No value is returned, but the `dest` array will store
	// 	the data read upon exit.
	int8_t xgReadBytes(uint8_t subAddress, uint8_t * dest, uint16_t count);
	...
	uint8_t SPIreadBytes(uint8_t csPin, uint8_t subAddress,
							uint8_t * dest, uint16_t count);
	int16_t I2Csendreceive(uint8_t address,void * buff_w,uint16_t buff_w_max, void * buff_r,uint16_t buff_r_max);
